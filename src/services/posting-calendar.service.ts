...
const resolveCalendarClientId = async (
  calendarId: string,
    ...
    return {
      ...
      previous_version: getPreviousPostVersion(versions),
        current_version_number:
          getCurrentPostVersion(versions)?.version_number ?? (Number(row.version_number || 1) || 1),
    };
  });
};
...
export const PostingCalendarService = {
  getPostVersionsMap: async (postIds: string[]) => {
    ...
    return attached.reduce((map, row: any) => {
  ...
  saveCalendarItemVersioned: async (payload: {
    ...
    const resolvedClientId = await resolveCalendarClientId(
      payload.calendar_id,
      payload.client_id || existingItem?.client_id || null
    );
    ...
    const basePayload: Record<string, unknown> = {
      ...
      status:
        payload.status ||
        (normalizedWorkflow === 'publicado'
          ? 'published'
          : normalizedWorkflow === 'agendado'
            ? 'scheduled'
      ...
      scheduled_date: payload.scheduled_date ?? existingItem?.scheduled_date ?? null,
      published_at: payload.published_at ?? existingItem?.published_at ?? null,
    ...
    if (existingItem?.id) {
      ...
      if (
        ...
        (isMissingSchemaEntityError(response.error, 'approval_notes') ||
          isMissingSchemaEntityError(response.error, 'scheduled_date') ||
          isMissingSchemaEntityError(response.error, 'published_at') ||
        ...
        delete fallbackPayload.approval_notes;
        delete fallbackPayload.scheduled_date;
        delete fallbackPayload.published_at;
        delete fallbackPayload.change_reason;
        delete fallbackPayload.change_log;
        delete fallbackPayload.is_current_version;
        delete fallbackPayload.parent_post_id;
        delete fallbackPayload.version_number;
    ...
    if (!postId) {
      void systemError({
        scope: 'posting_calendar',
        action: 'save_post_failed',
        clientId: resolvedClientId,
        tableName: 'posting_calendar_items',
        message: 'Nao foi possivel determinar o post salvo.',
        data: {
    ...
    void systemLog({
      scope: 'posting_calendar',
      action: existingItem?.id ? 'post_version_created' : 'post_created',
      clientId: resolvedClientId,
      tableName: 'posting_calendar_items',
      message: existingItem?.id
        ? 'Nova versão append-only criada no calendário.'
        : 'Post criado no calendário editorial.',
      data: {
        postId,
        calendarId: payload.calendar_id,
        versionNumber: nextVersionNumber,
        previousVersionNumber,
        workflowStatus: normalizedWorkflow,
      },
    });
